...
from flask import Flask, render_template, request, flash, redirect, session, g, jsonify, redirect, url_for
...
def add_like(message_id):
    """Add or remove a like for a warble."""
    ...
    if user.likes.filter(Likes.message_id == message.id).first():
        # User has already liked the warble, so remove the like
        like = user.likes.filter(Likes.message_id == message.id).first()
        db.session.delete(like)
        db.session.commit()
        flash('Warble unliked!', 'success')
    else:
        # User has not liked the warble, so add a like
